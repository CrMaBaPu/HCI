...
def get_closest_detection_and_distance(gaze_x: float, gaze_y: float, frame: int, yolo_data: pd.DataFrame, distance_threshold: float = 100.0):
    """
    Calculate the closest object class(es) and the distance to the closest bounding box.
    ...
    - distance_threshold (float): The threshold to consider an object as "near" (default is 100 pixels).
    ...
    for bbox_row in frame_bboxes.itertuples(index=False):
        ...
        # Only consider classes that are "near" or inside the bounding box (within distance threshold)
...
def process_yolo_gaze_data(gaze_data: pd.DataFrame, yolo_data: pd.DataFrame, target_classes: list, distance_threshold: float = 100.0) -> pd.DataFrame:
    """
    ...
    Returns:
    - pd.DataFrame: A DataFrame containing calculated gaze features for each object class.
    """
    ...
    for gaze_row in gaze_data.itertuples(index=False):
        ...
        closest_bboxes, min_distance = get_closest_detection_and_distance(gaze_x, gaze_y, frame, yolo_data, distance_threshold)
    ...
    # Prepare the final features data
    feature_data = []
    for cls in target_classes:
        feature_data.append({
            'most_frequent_class': most_frequent_class,
            'avg_duration_near_object': avg_durations.get(cls, 0),
            'avg_distance_to_closest_object': avg_distances.get(cls, 0.0),
        })
