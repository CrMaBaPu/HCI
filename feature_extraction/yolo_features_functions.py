...
def accumulate_class_detections(bboxes: list, target_classes: list) -> dict:
    """
    Accumulate the total number of detections for each target class across the segment.
    
    Args:
        bboxes (list): A list of bounding boxes for the current frame.
        target_classes (list): List of target classes to calculate the total detections for.
    
    Returns:
        dict: A dictionary with counts of detections per target class.
    """
    clscounts = defaultdict(int)  # Dictionary to store counts of detections per class
    for _, cls, _, _, _, _, _ in bboxes:
        if cls in target_classes:  # Only count the target classes
...
def process_yolo_data(yolo_data: pd.DataFrame, frame_width: float, frame_height: float) -> pd.DataFrame:
    ...
    #target_classes = ['car', 'bicycle', 'person']  # Only track these classes
    ...
    for frame, bboxes in grouped_by_frame.items():
        ...
        class_detections = accumulate_class_detections(bboxes, target_classes)  # Filter to target classes
        ...
        # Add max size for each target class
        features.update(max_class_size)

        # Accumulate the total number of detections per target class
        for cls in target_classes:
            # Check if the class has any detections in the current frame, otherwise set it to zero
            class_count = class_detections.get(cls, 0)
            segment_features[f'num_per_cls_{cls}'].append(class_count)
