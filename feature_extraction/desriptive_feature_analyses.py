...
def extract_metadata_from_path(file_path: str) -> tuple:
    ...
    criticality = "critical" if video_folder.startswith("crit_") else "uncritical" if video_folder.startswith("uncrit_") else "unknown"
    video_id = video_folder.split('_')[-1]
    
    return vehicle_type, criticality, video_id

def load_and_process_data(zip_file_path: str, output_folder: Path) -> pd.DataFrame:
